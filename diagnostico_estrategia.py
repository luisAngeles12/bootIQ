...
def evaluar_confianza_price_action(ctx, direccion):
    try:
        ...
        if direccion == "CALL":
            ...
            if direccion_tendencia == "ALCISTA" and fuerza_tendencia >= 50:
                score += 8
        ...
        if direccion == "PUT":
            ...
            if direccion_tendencia == "BAJISTA" and fuerza_tendencia >= 50:
                score += 8
...
def diagnosticar_base_estrategia(senal, ctx):
    try:
        ...
        if direccion == "call" and accion_precio == "CALL_ZONA_NEUTRA":
            fortaleza("CALL_ZONA_NEUTRA")

        if direccion == "put" and accion_precio == "PUT_ZONA_NEUTRA":
            fortaleza("PUT_ZONA_NEUTRA")
        ...
        if pa_tipo == "IMPULSO_ALCISTA_FUERTE":
            riesgo("IMPULSO_ALCISTA_FUERTE_DEBIL_HISTORICO")

        # =========================
        # TENDENCIA
        # =========================
        if tendencia_a_favor and 45 <= fuerza_tendencia <= 64:
            fortaleza("TENDENCIA_A_FAVOR_MODERADA")

        elif tendencia_a_favor and fuerza_tendencia >= 65:
        ...
        if "liquidity sweep" in patron:
            if pa_valido and nivel_pa in ["MEDIA", "ALTA"]:
        ...
        if "pullback" in patron:
            if tendencia_a_favor and 45 <= fuerza_tendencia <= 68 and pa_valido:
        ...
        if "continuación" in patron or "continuacion" in patron:
            if tendencia_a_favor and fuerza_tendencia >= 50 and pa_valido:
        ...
        # Clasificación menos pesimista.
        # Antes demasiadas señales terminaban como DEBIL.
        if fortalezas >= 3 and riesgos <= 2:
            diagnostico["base_estrategia"] = "FUERTE"
        elif riesgos >= 5 and fortalezas == 0:
            diagnostico["base_estrategia"] = "DEBIL"
        elif riesgos >= 4 and fortalezas <= 1:
            diagnostico["base_estrategia"] = "DEBIL"
        else:
