...
def evaluar_price_action_decision(evidencia):
    ...
    for ev in pa_evidencias:
        ...
        if tipo == "contradiccion_pa":
            ajuste_evidencias -= 6
            motivos.append(
                "PA: contradicción interna detectada."
            )
            continue
...
def evaluar_mercado_decision(evidencia):
    ...
    if (
        ...
        ajuste_evidencias += 2
        motivos.append(
            "Mercado: CALL con tendencia limpia."
        )
    ...
    if (
        ...
        ajuste_evidencias += 4
        motivos.append(
            "Mercado: PUT bajista fuerte y limpio."
        )
    ...
    if (
        ...
        ajuste_evidencias -= 4
        motivos.append(
            "Mercado: PUT bajista normal sin limpieza."
        )

    if "tendencia_debil" in tipos_mercado:
        ajuste_evidencias -= 3
        motivos.append(
            "Mercado: tendencia débil."
        )
...
def evaluar_decision_cerebro_unico(evidencia):
    ...
    fortalezas_base = _txt(
        evidencia.get("fortalezas_base", "")
    )
    ...
    motivos.extend(
        resultado_inferencia.get("motivos", [])
    )
    ...
    # PRICE ACTION
    # =========================
    resultado_pa = evaluar_price_action_decision(
        evidencia
    )

    ajuste_evidencias += resultado_pa.get(
        "ajuste",
        0
    )
    ...
    for ev in mercado_evidencias:
        ...
        tipo_mercado_ev = _txt(
            ev.get("tipo", "")
        )

        if tipo_mercado_ev:
            tipos_mercado.add(tipo_mercado_ev)
    ...
    if es_choch:
        if "choch_con_pa_a_favor" in subtipo_setup:
            ajuste_evidencias += 2
            motivos.append(
                "Estrategia: CHOCH con PA a favor."
            )

        if "choch_con_tendencia_debil" in riesgos_base:
            ajuste_evidencias -= 5
            motivos.append(
                "Estrategia: CHOCH con tendencia débil."
            )

        if "choch_sin_pa_valido" in riesgos_base:
            ajuste_evidencias -= 4
            motivos.append(
                "Estrategia: CHOCH sin PA válido."
            )
    ...
    confianza = round(
        max(
            ...
            min(
                100,
                confianza + ajuste_evidencias
            )
        ),
        2
    )

    ponderacion = calcular_ponderacion_estadistica(
        evidencia
    )
    ...
    if (
        ...
        motivos.append(
            "Cerebro único: confianza insuficiente."
        )
