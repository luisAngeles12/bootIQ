...
import motor_aprendizaje_historico as mah
...
# ============================================================
# F5.7-D5.5 — CONSENSO INTERMEDIO SOMBRA
# ============================================================
# HIPÓTESIS CONGELADAS:
#
# A_PA_RESCATE:
#   decisión oficial NO_OPERAR
#   + principal PA_DIRECCION
#   + mediana de TODAS las INTERMEDIAS utilizables >= 50
#
# B_SETUP_VETO:
#   decisión oficial autorizada
#   + principal CONTEXTO_SETUP
#   + mediana de TODAS las INTERMEDIAS utilizables < 50
#
# Solo telemetría. Nunca modifica la decisión oficial.
# ============================================================

D55_UMBRAL_CONSENSO = 50.0
D55_MEMORIA_RUTA = mah.RUTA_APRENDIZAJE
_D55_MEMORIA_CACHE = None
...
def crear_registro_resultado(
    ...
    registro = {
        ...
        # F5.7-D5.5 — consenso intermedio sombra.
        "d55_evaluada": bool(
            senal.get("d55_evaluada", False)
        ),
        "d55_reconstruccion_valida": bool(
            senal.get("d55_reconstruccion_valida", False)
        ),
        "d55_error_probabilidad": senal.get(
            "d55_error_probabilidad", 0.0
        ),
        "d55_consenso_disponible": bool(
            senal.get("d55_consenso_disponible", False)
        ),
        "d55_consenso_intermedio": senal.get(
            "d55_consenso_intermedio", 0.0
        ),
        "d55_fuentes_intermedias": senal.get(
            "d55_fuentes_intermedias", 0
        ),
        "d55_intermedias_positivas": senal.get(
            "d55_intermedias_positivas", 0
        ),
        "d55_intermedias_negativas": senal.get(
            "d55_intermedias_negativas", 0
        ),
        "d55_principal_nivel_reconstruido": senal.get(
            "d55_principal_nivel_reconstruido", ""
        ),
        "d55_principal_clave_reconstruida": senal.get(
            "d55_principal_clave_reconstruida", ""
        ),
        "d55_probabilidad_reconstruida": senal.get(
            "d55_probabilidad_reconstruida", 0.0
        ),
        "d55_pa_rescate_sombra": bool(
            senal.get("d55_pa_rescate_sombra", False)
        ),
        "d55_setup_veto_sombra": bool(
            senal.get("d55_setup_veto_sombra", False)
        ),
        "d55_accion": senal.get(
            "d55_accion", "SIN_CAMBIO"
        ),
        ...
        "d43_sombra_accion": senal.get(
            "d43_sombra_accion",
            "",
        ),

        "d43_sombra_objetivo_espera": senal.get(
            "d43_sombra_objetivo_espera",
            -1,
        ),

        "d43_sombra_protocolo": senal.get(
            "d43_sombra_protocolo",
            "",
        ),

        "d43_sombra_idx_entrada": senal.get(
            "d43_sombra_idx_entrada",
            -1,
        ),

        "d43_sombra_espera_velas": senal.get(
            "d43_sombra_espera_velas",
            -1,
        ),

        "d43_sombra_motivo": senal.get(
            "d43_sombra_motivo",
            "",
        ),

        "d43_sombra_encuentra_entrada": bool(
            senal.get(
                "d43_sombra_encuentra_entrada",
                False,
            )
        ),

        "d43_sombra_cumple_minimo": bool(
            senal.get(
                "d43_sombra_cumple_minimo",
                False,
            )
        ),

        "d43_sombra_resultado": senal.get(
            "d43_sombra_resultado",
            "",
        ),

        "d43_sombra_relacion_oficial": senal.get(
            "d43_sombra_relacion_oficial",
            "",
        ),
        # ==================================================
...
def evaluar_d43_timing_minimo_sombra(
    senal,
    velas,
    idx,
    idx_entrada_oficial,
    ...
    - la decisión del Cerebro;
    - la entrada oficial;
    - el estado oficial de la operación;
    - el aprendizaje.

    Hipótesis estudiada:

        ESPERAR_2:
            oficial -> puede entrar +1 o +2
            sombra  -> solo puede comenzar en +2

        ESPERAR_3:
            oficial -> puede entrar +1, +2 o +3
            sombra  -> solo puede comenzar en +3

    Conserva el mismo límite máximo de la ventana.
    Por tanto, se modifica exclusivamente el INICIO temporal
    de búsqueda y se mantienen intactas las confirmaciones
    técnicas de cada protocolo.
    """
    ...
    if decision != "OPERAR_CON_PROTOCOLO":
        return salida
    ...
    salida["objetivo_espera"] = objetivo_espera

    senal_sombra = copy.deepcopy(
        senal
    )
    ...
    if idx_sombra is None:

        if idx_entrada_oficial is None:
            salida[
                "relacion_oficial"
            ] = "AMBAS_SIN_ENTRADA"
    ...
    salida["idx_entrada"] = idx_sombra
    salida["espera_velas"] = espera_sombra

    cumple_minimo = (
        espera_sombra >= objetivo_espera
    )

    salida["cumple_minimo"] = cumple_minimo

    # Protección diagnóstica:
    # si alguna ruta interna esquiva la ventana temporal,
    # no la contamos como una entrada válida D4.3.
    if not cumple_minimo:
        salida["motivo"] = (
            "D43_SOMBRA_ENTRADA_ANTES_MINIMO | "
            + str(motivo_sombra)
        )

        salida[
            "relacion_oficial"
        ] = "BYPASS_TEMPORAL_SOMBRA"

        return salida

    if idx_sombra + 1 >= len(velas):
        salida["motivo"] = (
            "D43_SOMBRA_SIN_VELA_RESULTADO"
        )
        return salida
    ...
    # ========================================================
    # RELACIÓN CON ENTRADA OFICIAL
    # ========================================================

    if idx_entrada_oficial is None:
        salida[
            "relacion_oficial"
        ] = "SOMBRA_ENCUENTRA_ENTRADA"

    elif idx_sombra == idx_entrada_oficial:
        salida[
            "relacion_oficial"
        ] = "MISMA_ENTRADA"
        ...
        salida[
            "relacion_oficial"
        ] = "ENTRADA_ANTERIOR_INESPERADA"

    return salida

# ============================================================
# F5.7-D5.5 — MOTOR DE TELEMETRÍA SOMBRA
# ============================================================

def evaluar_d55_consenso_sombra(senal, decision_oficial):
    global _D55_MEMORIA_CACHE

    salida = {
        "d55_evaluada": True,
        "d55_reconstruccion_valida": False,
        "d55_error_probabilidad": 0.0,
        "d55_consenso_disponible": False,
        "d55_consenso_intermedio": 0.0,
        "d55_fuentes_intermedias": 0,
        "d55_intermedias_positivas": 0,
        "d55_intermedias_negativas": 0,
        ...
        "d55_principal_clave_reconstruida": "",
        "d55_probabilidad_reconstruida": 0.0,
        "d55_pa_rescate_sombra": False,
        "d55_setup_veto_sombra": False,
        "d55_accion": "SIN_CAMBIO",
    }

    try:
        if _D55_MEMORIA_CACHE is None:
            _D55_MEMORIA_CACHE = mah.cargar_aprendizaje(
                D55_MEMORIA_RUTA
            )

        fuentes, _ = mah._buscar_fuentes_aprendizaje(
            senal,
            _D55_MEMORIA_CACHE,
        )

        principal = (
            mah._seleccionar_fuente_principal(fuentes)
            or {}
        )

        combinado = mah._combinar_fuentes(fuentes)
        ...
        prob_oficial = mah._numero(
            senal.get("probabilidad_estimada"),
            mah.PRIOR_WINRATE,
        )
        ...
        prob_rec = mah._numero(
            combinado.get("probabilidad_estimada"),
            mah.PRIOR_WINRATE,
        )

        error_prob = abs(
            prob_rec - prob_oficial
        )

        salida["d55_principal_nivel_reconstruido"] = nivel_rec
        salida["d55_principal_clave_reconstruida"] = clave_rec
        salida["d55_probabilidad_reconstruida"] = round(
            prob_rec,
            2,
        )
        salida["d55_error_probabilidad"] = round(
            error_prob,
            4,
        )
        ...
        valida = bool(
            nivel_oficial
            and clave_oficial
            and nivel_rec == nivel_oficial
            and clave_rec == clave_oficial
            and error_prob <= 0.02
        )

        salida["d55_reconstruccion_valida"] = valida

        if not valida:
            salida["d55_accion"] = "RECONSTRUCCION_NO_VALIDA"
            return salida
        ...
        intermedias = [
            ...
            if (
                ...
                and mah._entero(
                    fuente.get("total"),
                    0,
                ) >= mah.MIN_MUESTRA_APORTE
            )
        ]

        probs = [
            mah._numero(
                fuente.get("probabilidad_ajustada"),
                mah.PRIOR_WINRATE,
            )
            for fuente in intermedias
        ]

        salida["d55_fuentes_intermedias"] = len(probs)

        salida["d55_intermedias_positivas"] = sum(
            1 for p in probs
            if p >= D55_UMBRAL_CONSENSO
        )

        salida["d55_intermedias_negativas"] = sum(
            1 for p in probs
            if p < D55_UMBRAL_CONSENSO
        )

        if not probs:
            salida["d55_accion"] = "SIN_FUENTES_INTERMEDIAS"
            return salida
        ...
        salida["d55_consenso_disponible"] = True
        salida["d55_consenso_intermedio"] = consenso

        # A — definición congelada.
        if (
            decision_oficial == "NO_OPERAR"
            and nivel_oficial == "PA_DIRECCION"
            and consenso >= D55_UMBRAL_CONSENSO
        ):
            salida["d55_pa_rescate_sombra"] = True
            salida["d55_accion"] = "A_PA_RESCATE"

        # B — definición congelada.
        elif (
            decision_oficial
            in {"OPERAR", "OPERAR_CON_PROTOCOLO"}
            and nivel_oficial == "CONTEXTO_SETUP"
            and consenso < D55_UMBRAL_CONSENSO
        ):
            salida["d55_setup_veto_sombra"] = True
            salida["d55_accion"] = "B_SETUP_VETO"
...
def ejecutar_backtest(datasets):
    ...
    for i in range(180, max_len - 2, PASO_RONDA):
        ...
        for senal in senales_ronda[:MAX_SENALES_POR_RONDA]:
            ...
            # ========================================================
            # F5.7-D5.5 — TELEMETRÍA SOMBRA
            # ========================================================
            # Observa la decisión ya tomada.
            # No modifica operar, protocolo ni entrada.
            # ========================================================

            d55 = evaluar_d55_consenso_sombra(
                senal,
                decision_oficial,
            )
            ...
            if (
                ...
                d43_sombra = (
                    evaluar_d43_timing_minimo_sombra(
                        senal,
                        velas,
                        idx,
                        idx_entrada,
                ...
                senal["d43_sombra_aplica"] = (
                    d43_sombra["aplica"]
                )

                senal["d43_sombra_accion"] = (
                    d43_sombra["accion"]
                )
                ...
                senal["d43_sombra_protocolo"] = (
                    d43_sombra["protocolo"]
                )

                senal["d43_sombra_idx_entrada"] = (
                    d43_sombra["idx_entrada"]
                    if d43_sombra["idx_entrada"]
                    is not None
                    else -1
                )

                senal["d43_sombra_espera_velas"] = (
                    d43_sombra["espera_velas"]
                )

                senal["d43_sombra_motivo"] = (
                    d43_sombra["motivo"]
                )
                ...
                senal["d43_sombra_resultado"] = (
                    d43_sombra["resultado"]
                )
...
def guardar_resultados(resultados):
    campos = [
        ...
        # F5.7-D5.5 — consenso intermedio sombra.
        "d55_evaluada",
        "d55_reconstruccion_valida",
        "d55_error_probabilidad",
        "d55_consenso_disponible",
        "d55_consenso_intermedio",
        "d55_fuentes_intermedias",
        "d55_intermedias_positivas",
        "d55_intermedias_negativas",
        "d55_principal_nivel_reconstruido",
        "d55_principal_clave_reconstruida",
        "d55_probabilidad_reconstruida",
        "d55_pa_rescate_sombra",
        "d55_setup_veto_sombra",
        "d55_accion",
...
def imprimir_d43_timing_minimo_sombra(
    resultados,
):
    """
    F5.7-D4.3B.

    Compara el timing oficial de motor_protocolos contra
    una sombra que obliga a respetar la espera mínima:

        ESPERAR_2 -> no antes de +2
        ESPERAR_3 -> no antes de +3
    ...
    filas = [
        r
        for r in resultados
        if r.get("d43_sombra_aplica") is True
    ]

    print(
        "\n===== F5.7-D4.3B TIMING MINIMO SOMBRA — "
        + str(MODO_EXPERIMENTO)
        + " ====="
    )

    if not filas:
        print(
            "No hay señales aplicables a D4.3B."
        )
        print(
            "=============================================="
        )
    ...
    # ========================================================
    # BASELINE PROTOCOLO VS SOMBRA
    # ========================================================

    ot, ow, ol, owr = resumen(
        oficiales,
        "resultado",
    )

    st, sw, sl, swr = resumen(
        sombra,
        "d43_sombra_resultado",
    )

    print("\n--- PROTOCOLO OFICIAL VS SOMBRA ---")
    ...
    # ========================================================
    # EFECTO SOBRE BACKTEST COMPLETO
    # ========================================================

    dt, dw, dl, dwr = resumen(
        directas,
        "resultado",
    )

    baseline_total = dt + ot
    baseline_win = dw + ow
    baseline_loss = dl + ol
    ...
    if not transiciones:
        print("No hay operaciones comparables.")
    ...
    for accion, grupo in sorted(
        ...
        aot, aow, aol, aowr = resumen(
            oficial_grupo,
            "resultado",
        )

        ast, asw, asl, aswr = resumen(
            sombra_grupo,
            "d43_sombra_resultado",
        )
    ...
    for r in filas:
        ...
        protocolos.setdefault(
            protocolo,
            [],
        ).append(r)

    for protocolo, grupo in sorted(
        protocolos.items(),
        key=lambda x: -len(x[1]),
    ):
...
def imprimir_resumen(resultados):
    ...
    imprimir_c8_evento_tecnico(resultados)
        # F5.7-D4.3B — timing mínimo sombra.
    imprimir_d43_timing_minimo_sombra(
        resultados
    )
    # C-C2 oficial.
